...
def download_building_geometry(quad_keys, aoi_shape, output_building_name):
    ...
    db = pd.read_csv("https://minedbuildings.blob.core.windows.net/global-buildings/dataset-links.csv")
    idx = 0
    combined_rows = []
    ...
    # 使用更新后的 schema 写入文件
    with fiona.open(output_building_path, "w", driver="GeoJSON", crs="EPSG:4326", schema=schema) as f:
...
def get_terrain(output_complete_terrain_name, output_trimmed_terrain_name,
